<!-- XSLT to display an QRD/SmPC bundle/composition as HTML - Rik 2021-01-21 -->
...
<xsl:template match="xhtml:span[@id]">
	<xsl:variable name="id" select="concat('#',@id)"/>
	<span style="white-space: pre-line;">
		<xsl:variable name="text">
			<xsl:value-of select="//f:*[f:extension[@url='http://hl7.org/fhir/StructureDefinition/originalText']/f:valueUrl/@value=$id]/@value"/>
		</xsl:variable>
		<xsl:if test="string-length($text)=0">[warning - could not resolve reference to <xsl:value-of select="$id"/>]</xsl:if>
...
@media all {
    .header { font-size:12pt; text-weight:bold; text-align:center; margin-bottom:-15px; }
